...
- **All eleven graded runs are in one committed table:**
  [`examples/isolated-runs-compare.html`](examples/isolated-runs-compare.html) —
  every row behind findings 12 and 13 side by side, generated by
  `cc_trace compare` from the same `report.json` files the numbers are quoted
  from. It is safe to commit precisely because these runs happened *inside
  containers*: it contains no host paths at all. `opus sympy r2` is the polling
  artifact, and shows up there exactly as it should — `poll% 0.90` and a purity
  of `0.97!`.
